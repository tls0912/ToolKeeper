...
左側工具列預設展開並記住手動選擇；收合時維持窄版，只顯示 Icon。
...
點擊工具列頂部的展開按鈕後，顯示名稱、功能說明與適用的快捷鍵：
...
再次點擊同一個收合按鈕才縮回。滑鼠移入或移開均不改變展開狀態。
...
- 未儲存 Tab 在檔名前加 `●`
- 2026-09-22 提示位置修訂：處理中等短暫操作提示顯示在視窗上方中央，與標題列保留 24 px 間隔；全螢幕亦保留此區域。
...
- 檔名過長時採尾端省略
- Hover Tab 時顯示完整檔案路徑
- Tab 顯示完整副檔名，例如 `README.md`
- 不同資料夾出現同名檔案時，只有同名情況才附加父資料夾名稱
- 目前 Tab 的關閉 `×` 永遠顯示
- 其他 Tab 的 `×` 僅 Hover 時顯示
- 支援拖曳重排 Tab
- 滑鼠中鍵點擊 Tab 直接關閉
- Tab 雙擊不綁定特殊功能
- 不支援 Pin Tab
...
- Tab 超出可用空間後，超出項目進入下拉清單
- 下拉清單依目前 Tab 順序排列
- 超過 10 個 Tab 時，下拉清單才顯示搜尋框
- 未儲存文件在下拉清單中同樣顯示 `●`

### 關閉後切換

- 關閉目前 Tab 後，切換到最近使用過的 Tab
- `Ctrl+Tab` 仍依 Tab 左到右順序切換

### Tab 與視窗

- Tab 可拖出目前視窗形成新的 MarkPad 視窗
- 不同 MarkPad 視窗之間可互相拖曳 Tab
- 拖移後保留原本 Preview / Edit 狀態
- 拖出形成新視窗時，複製原視窗大小
...
- 收合寬度：**44 px**
- 展開寬度依語系與文字自適應
- 展開基準寬度：**200 px**，隨 UI 字級增加以容納文字
- 展開後顯示 **Icon + 功能名稱 + 簡短說明 + 適用的快捷鍵**
- 收合時僅顯示 Icon
- 分組使用分隔線
- 分組標題只在展開時顯示
- 收合時提供 Tooltip
...
### 手動展開／收合（2026-09-22 使用者修訂）

- 工具列最上方永遠保留同一顆展開／收合按鈕
- 新安裝與重設設定時預設展開；之後記住手動選擇。
- 展開時使用細圓角捲軸與透明軌道，移除上下箭頭；保留滾輪、拖曳滑塊及點擊軌道翻頁。
- 不使用 Hover 自動展開或離開自動收合
- 不另外提供 Pin 按鈕；狀態由使用者手動切換
- 展開時實際分配側欄寬度，功能說明不得被窄欄裁切
- 展開／收合時按鍵圖示位置保持穩定
- 記住使用者選擇，下次啟動維持上次狀態
...
- 分類：
  ...
  - Preview
  - File
- Recent Files 從 More 進入後，以次級 Flyout 顯示
...
- 字級操作使用 `−  16  +`
- 允許直接輸入字級數值
- 分成「UI 介面」「內文預覽」「內文編輯器」三組，分別記錄字型與字級。
- UI 介面字級為 10–20；內文預覽與內文編輯器為 8–72。變更立即生效，不修改 Markdown 文件。
- 三組字級預設均為 16。UI／預覽字型依語系自動選擇；編輯器預設 Cascadia Mono。
- 英文名稱為 User interface／Document preview／Document editor；日文為 UI インターフェース／本文プレビュー／本文エディター。
- 小視窗或較大介面字級時，字型設定可捲動，以存取全部三組設定。
...
- Theme 提供：
  ...
  - Dark
  - System
- 2026-09-22 配色修訂：Light 使用淡灰色視窗與工具列、較淺的灰色文件底色，Preview 與 Edit 一致。
- 後續配色調整：文件底色採原介面灰色 `#E8EBEF`，外框與工具列加深為 `#D9DEE4`；Preview 與 Edit 同步。
- More 主選單、各層子選單、反白、勾選與停用文字都跟隨 Theme，不混用 Windows 的亮色選單樣式。
- UI 語系切換後立即生效，不需重新啟動
...
- 1200 px 模式只修改寬度，不修改高度
- 半螢幕使用 Windows 實際可用工作區
- 再次選擇目前尺寸時，回到上一個尺寸
- More 提供「記住視窗尺寸」設定，預設開啟；未記錄高度時採工作區高度的 85%。

### F11 Full Screen

- 左側工具列保留手動展開／收合入口，不隨滑鼠自動顯示或隱藏
- 頂部分頁列預設隱藏，滑鼠移到頂部時浮出
- 浮出的頂部列不遮擋左側工具列
- 頂部列浮出後約 **1 秒**自動隱藏；左側工具列維持手動選擇的狀態
- `Esc`：
  - 若有 Search、圖片放大等浮層，先關閉浮層
  - 無浮層時再退出 Full Screen
...
- 雙擊 Preview 一般文字沿用 WebView 的文字選取行為
- 不因雙擊自動切 Edit
...
- 切換到 Edit Mode
- 定位到最接近的 Markdown 原始段落

### Copy 行為

- Preview 選取文字後：
  - `Ctrl+C` 複製純文字
  - `Ctrl+Shift+C` 複製 Markdown
- 右鍵提供 `Copy as Markdown`

### Link 行為

- 一般外部網址：直接交給系統預設瀏覽器
- 外部連結右鍵提供 `Copy Link`
- `mailto:`：不啟動郵件程式，改為複製 Email
- 內部 `.md` 連結若目標不存在，顯示簡短 `File not found`

### Heading Folding

- H1～H6 均支援 Preview 內容折疊
- 折疊只影響顯示，不修改原始 Markdown
...
- 第一次啟動預設跟隨 Windows 顯示語言（0.1.6 使用者修訂）
- Language 提供「跟隨系統」、英文、繁體中文及日文；跟隨模式保留為設定值，不寫死當次解析的語言。
- 系統語言不在支援範圍時使用英文；系統語言變更後，重新開啟 MarkPad 時重新解析。
- 語系切換立即生效，不需重新啟動
...
- 回復上述預設偏好與「跟隨系統」語言模式，不再次要求選擇語系；最近檔案清單清空
- 執行前顯示確認視窗
...
- 工作列標題固定為 `MarkPad`；文件名稱與未儲存標記保留在程式內分頁。
- 執行檔、工作列與視窗使用 MarkPad 多尺寸 ICO 圖示。
- 不自行干預工作列分組行為
- 交給 Windows 預設處理
